...
| `effort`               | `string`            | no       | _(none)_                          | Phase 4.8 stub (pending Phase 5.4): per-run effort / reasoning-depth override. **Currently accepted-but-not-consumed** — the value is stored on the resolved options struct so the field surface is stable for the `spawn_subagent` per-subagent `effort` override, but the OR `callModel` call does not yet forward it. Real wiring lands in Phase 5.4 (gated on spike 5.S3 — whether OR's API accepts an effort parameter at all).                                                                    |
...
| `effort`           | `string`         | no       | parent's `effort`          | Phase 4.8 (stub): per-subagent effort override. Currently a no-op pass-through — stored on the child run but not consumed until Phase 5.4. |
...
- **Sequential only.** Parallel subagent execution is Phase 4.9. Per-subagent `model` / `permission_mode` / `allowed_tools` / `disallowed_tools` / `effort` overrides shipped in Phase 4.8 (see the schema table above); `effort` remains a stored-but-no-op stub pending Phase 5.4.
- **Ignored with custom `tools`.** When the parent run is constructed with a custom `tools` array, `enableSubagents` is a no-op (callers wire their own `spawn_subagent` via `spawnSubagentTool({ runSubagent, … })` if they need it).
