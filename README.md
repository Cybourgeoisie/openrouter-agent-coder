...
The bundled tool preset — `allTools({ cwd, signal })` returns the eleven client tools below, context-bound. It is the default value for `OpenRouterAgentRun`'s `tools` option; you only need to reference it directly when composing a custom tool array.
