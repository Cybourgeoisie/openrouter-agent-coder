...
class OpenRouterAdapter implements AgentProvider {
  ...
  constructor(private opts: { apiKey?: string; baseUrl?: string; model?: string; env?: Record<string,string> }) {}

  query(req: AgentQueryRequest): AgentQuery {
    const run = new OpenRouterAgentRun({ ...this.opts, ...translateOptions(req.options), prompt: req.prompt });
...
function toolDefToOrTool(def: ToolDefinition): OrTool {
  return {
    ...
    execute: async (args) => {
      ...
      return result.content.map(b => b.type === "text" ? b.text : `[image:${b.mimeType}]`).join("\n");
    },
  };
}
```
...
- `PreToolUse` — already overlaps with `canUseTool` (§5). The hook is for *audit/side-effects*; `canUseTool` is for *decisions*. Wire both.
...
The existing parity doc lists 26 missing features. The subset that callboard *needs* (and this plan addresses) is a smaller list — most parity-doc P2 items (Skills, slash commands, plugins, NotebookEdit, ToolSearch, Monitor) are not callboard requirements.
...
Net: callboard-compat work is a *subset* of full parity work, with a different ordering. Permissions + hooks + custom-tools + event-stream are the load-bearing four.
