...
| 5.6       | Slash commands (post-5.7)                          | ~~5–8h~~ **3–5h**    | Flat-file `.claude/commands/<name>.md` discovery + `/<name>` resolution. Per [spike 5.S4](./spikes/5.S4-skills-commands-plugins.md), commands are degenerate skills — reuses 5.7's loader, frontmatter parser, and substitution helper. Build **second** in the new 5.7 → 5.6 → 5.8 order.                                                                                                                                                                                                                                                       |
| 5.7       | Skills system (post-5.S4)                          | ~~8–12h~~ **10–14h** | `.claude/skills/<name>/SKILL.md` discovery + `Skill` built-in tool + listing-block system-prompt injection + shared substitution helper (`$ARGUMENTS`, `${CLAUDE_*}`, `` !`cmd` ``) + `context: fork` wiring into existing subagent path + `allowed-tools` permission integration. **Build first** per [spike 5.S4](./spikes/5.S4-skills-commands-plugins.md) — superset of 5.6.                                                                                                                                                                 |
| 5.8       | Plugins (post-5.7, 5.6, 5.2.5)                     | ~~10–15h~~ **8–12h** | Plugin manifest (`.claude-plugin/plugin.json`) + auto-discovery + path-aggregation composition (replace-vs-add semantics) + `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` substitution. v1 defers marketplace fetcher, `userConfig` runtime, `channels`, `dependencies`, `${CLAUDE_PLUGIN_DATA}` cleanup, themes/monitors — see [spike 5.S4](./spikes/5.S4-skills-commands-plugins.md). Builds **last** since it composes 5.6 + 5.7.                                                                                                         |
