  ...
  return tool({
    ...
    inputSchema: z.object({
      ...
      end_line: z
        ...
        .min(1)
        .describe(
          'Last line to return (1-indexed, inclusive). Omit to read to the end of the file.',
        )
        .optional(),
    }),
    execute: async ({ path, start_line, end_line }) => {
      ...
      if (estimatedTokens > MAX_TOKENS_ESTIMATE) {
        throw new Error(
          `File content (~${estimatedTokens} estimated tokens) exceeds maximum allowed tokens (${MAX_TOKENS_ESTIMATE}). ` +
      ...
      if (truncatedByDefault) {
        return {
          ...
          truncated: true,
          notice:
            `File was too large and has been truncated to the first ${DEFAULT_LINE_LIMIT} lines. ` +
