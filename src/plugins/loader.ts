...
import type { McpServerConfig, McpStdioServerConfig, McpHttpServerConfig } from '../mcp/config.js';
 ...
 */
export async function loadPlugins(opts: LoadPluginsOptions): Promise<LoadedPlugin[]> {
...
async function loadOnePlugin(
  root: string,
  home: string,
  logger: LoadPluginsOptions['logger'],
  ...
  if (await pathExists(manifestPath)) {
    ...
    try {
      ...
      throw new Error(
        `plugin manifest at ${manifestPath} is not valid JSON: ${(err as Error).message}`,
        { cause: err },
      );
    }
    ...
    if (!result.success) {
      throw new Error(
        `plugin manifest validation failed at ${manifestPath}: ${result.error.issues[0]?.message ?? 'unknown'}`,
      );
    }
    ...
    if (!result.success) {
      throw new Error(
        `auto-discovered plugin name "${dirName}" is invalid (${result.error.issues[0]?.message ?? 'unknown'})`,
      );
    }
    manifest = result.data as PluginManifest;
  }
  ...
  const commandRoots = resolveReplaceableRoots(
    root,
    manifest.commands,
    PLUGIN_DEFAULT_PATHS.commands,
  );
 ...
 */
function resolveSkillRoots(root: string, manifest: PluginManifest): string[] {
 ...
 */
function resolveReplaceableRoots(
  root: string,
 ...
 */
async function resolveHookConfigs(
  root: string,
  manifest: PluginManifest,
  logger: LoadPluginsOptions['logger'],
): Promise<PluginHookConfig[]> {
  const out: PluginHookConfig[] = [];
 ...
 */
function extractHookMap(parsed: unknown): Record<string, unknown> | undefined {
...
async function resolveMcpServers(
  ...
  const envelope = raw as { mcpServers?: Record<string, unknown> } | null;
  const inner = envelope?.mcpServers ?? (raw as Record<string, unknown>);
  ...
  for (const [serverName, entry] of Object.entries(inner)) {
    ...
    if (typeof e.command === 'string') {
      ...
      if (Array.isArray(e.args)) cfg.args = e.args.map(String);
