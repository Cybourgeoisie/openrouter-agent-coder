...
import { renderSkillBody } from './substitution.js';
import { skillFrontmatterSchema, } from './spec.js';
 ...
 */
export function createSkillLoader(opts) {
    let cache = null;
    ...
    return {
        ...
        async render(name, ctx) {
            ...
            const effectiveCtx = ctx.skillDir !== undefined ? ctx : { ...ctx, skillDir: dirname(skill.location) };
...
async function discoverSkills(opts) {
    ...
    if (opts.disableUserSkills !== true) {
        ...
        for (const skill of await discoverScopedSkills(join(homeDir, '.claude', 'skills'), 'user', opts.logger)) {
    ...
    if (opts.pluginRoots && opts.pluginRoots.length > 0) {
        for (const { name: pluginName, root, skillsDir } of opts.pluginRoots) {
            for (const skill of await discoverScopedSkills(skillsDir ?? join(root, 'skills'), 'plugin', opts.logger, pluginName)) {
                out.set(skill.name, skill);
            }
        }
    }
 ...
 */
async function discoverProjectSkills(cwd, logger) {
    const out = [];
 ...
 * canonical shape).
 */
async function discoverScopedSkills(root, source, logger, pluginName) {
 ...
 */
export function parseSkillFile(raw, expectedDirName) {
    ...
    if (fm.name !== basename(expectedDirName)) {
        throw new Error(`frontmatter \`name\` (${fm.name}) does not match parent directory (${expectedDirName})`);
    }
...
function findClosingFence(raw, searchFrom) {
    let pos = searchFrom;
 ...
 * Exported for tests.
 */
export function normalizeFrontmatterKeys(raw) {
    const out = {};
