 ...
 */
export function skillTool(opts, ctx = DEFAULT_TOOL_CONTEXT) {
    ...
    return tool({
        ...
        inputSchema: z.object({
            name: z
                .string()
                .min(1)
                .describe('Qualified skill name. Use the name shown in the Available Skills listing — plugin skills appear as `<plugin>:<skill>`.'),
            arguments: z
                .string()
                .optional()
                .describe('Optional argument string (shell-quoted). Passed to the skill body via $ARGUMENTS / $N / $name substitution.'),
        }),
        execute: async ({ name, arguments: argString }) => {
 ...
 */
async function runForked(skill, args, opts, ctx) {
    if (!opts.runSubagent) {
        opts.logger?.('warn', `skill "${skill.name}" requested context:fork but no runSubagent is wired`);
    ...
    if (skill.frontmatter.agent !== undefined &&
        opts.knownSubagentTypes &&
        opts.knownSubagentTypes.length > 0 &&
        !opts.knownSubagentTypes.includes(skill.frontmatter.agent)) {
        opts.logger?.('warn', `skill "${skill.name}" requested unknown agent type; falling back to general-purpose`, { requested: skill.frontmatter.agent, known: opts.knownSubagentTypes });
    }
 ...
 */
export function buildSkillListing(skills, budgetChars) {
    if (skills.length === 0)
        return '';
...
function buildToolDescription(visible) {
    const base = 'Invoke a discovered skill by name. The skill body is substituted (variables + positional/named arguments + inline/fenced shell) and the rendered text is returned as the tool result. The full list of available skills (with descriptions and `when_to_use` hints) is in the `## Available Skills` block of the system instructions — pick a name from there. Use this when a skill explicitly fits the current subtask rather than re-deriving the same behavior from scratch.';
