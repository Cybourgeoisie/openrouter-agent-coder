...
export function globTool(ctx = DEFAULT_TOOL_CONTEXT) {
    return tool({
        name: 'glob',
        description: 'Find files by glob pattern across a directory tree. Patterns: `**/*.ts` (recursive), `src/**/*.test.ts` (scoped), `*.md` (flat). `*` matches anything except `/`; `?` matches a single non-`/` character; `[a-z]` character classes are supported. Returns relative paths sorted lexicographically, capped at 1000 matches. Skips node_modules, dist, coverage, and hidden files/dirs.',
        inputSchema: z.object({
            pattern: z
                .string()
                .describe('Glob pattern to match against relative paths from `path` (e.g. "**/*.ts", "src/**/*.test.ts", "*.md").'),
            path: z
                .string()
        ...
        execute: async ({ pattern, path, case_sensitive }) => {
            if (ctx.signal?.aborted)
                throw new Error('glob cancelled');
