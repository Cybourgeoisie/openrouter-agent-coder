...
export function writeFileTool(ctx = DEFAULT_TOOL_CONTEXT) {
    return tool({
        name: 'write_file',
        description: 'Write content to a file, creating it if it does not exist or overwriting if it does. Parent directories are created automatically.',
        inputSchema: z.object({
            ...
            checkpoint: z
                .boolean()
                .optional()
                .describe('When true, snapshot the file under the session checkpoints directory before writing. Overrides the run-level default.'),
        }),
        execute: async ({ path, content, checkpoint }) => {
            if (ctx.signal?.aborted)
                throw new Error('write_file cancelled');
