...
export function editFileTool(ctx = DEFAULT_TOOL_CONTEXT) {
    return tool({
        name: 'edit_file',
        description: 'Edit a file by replacing an exact string match with new content. The old_string must appear exactly once in the file. Use read_file first to see the current content.',
        ...
        execute: async ({ path, old_string, new_string, checkpoint }) => {
            ...
            if (occurrences > 1) {
                throw new Error(`old_string found ${occurrences} times in ${path} — it must be unique. Provide more surrounding context.`);
            }
