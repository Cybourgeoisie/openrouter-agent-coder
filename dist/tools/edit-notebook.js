...
function stringToSourceArray(source) {
    if (source === '')
        return [];
...
export function editNotebookTool(ctx = DEFAULT_TOOL_CONTEXT) {
    return tool({
        name: 'edit_notebook',
        description: "Edit a Jupyter notebook (.ipynb) by manipulating cells. Operations: `replace_source` (rewrite a cell's source), `insert` (add a new cell), `delete` (remove a cell), `change_type` (toggle code/markdown). Untouched cells and notebook metadata are preserved. Returns `{ ok: true, cells: <count> }` on success or `{ error: ... }` on a validation / IO failure.",
        inputSchema: z.object({
            ...
            cell_index: z
                ...
                .int()
                .nonnegative()
                .describe('0-based cell index. For `insert`, the position where the new cell lands (0 = prepend, cells.length = append).'),
            new_source: z
                .string()
                .optional()
        ...
        execute: async ({ path, operation, cell_index, new_source, new_cell_type, }) => {
            ...
            if (op === 'replace_source') {
                ...
                const target = cells[cell_index];
                const currentType = target.cell_type;
            ...
            try {
                ...
                return { error: `failed to write notebook: ${err.message}` };
            }
            return { ok: true, cells: cells.length };
        },
    });
}
//# sourceMappingURL=edit-notebook.js.map