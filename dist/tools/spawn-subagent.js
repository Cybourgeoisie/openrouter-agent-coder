 ...
 */
export const EFFORT_VALUES = ['xhigh', 'high', 'medium', 'low', 'minimal', 'none'];
/**
...
export const SPAWN_SUBAGENT_INPUT_SCHEMA = z.object({
    ...
    tools: z
        .array(z.string())
        .optional()
        .describe("Optional whitelist of tool names the subagent may use (e.g. ['read_file', 'grep_files']). Unknown names are silently dropped. Omit to inherit the parent's full tool pool."),
    instructions: z
        .string()
        .optional()
    ...
    model: z
        ...
        .min(1)
        .optional()
        .describe("Per-subagent model override (e.g. `'~anthropic/claude-sonnet-latest'`). Omit to inherit the parent's resolved `model`."),
    permission_mode: z
    ...
    return tool({
        ...
        execute: async ({ description, tools, instructions, max_turns, max_budget_usd, model, permission_mode, allowed_tools, disallowed_tools, effort, }, execCtx) => {
            ...
            if (childDepth >= maxDepth) {
                ...
                const errResult = { status: 'error', reason, text: '' };
            ...
            const parentSignal = execCtx?.signal ?? ctx.signal;
            ...
            try {
                ...
                const errResult = { status: 'error', reason: message, text: '' };
                await opts.onSubagentLifecycle?.('SubagentEnd', {
                    event: 'SubagentEnd',
                    parentSessionId: opts.parentSessionId,
                    subagentSessionId,
 ...
 */
async function runPool(items, cap, worker) {
 ...
 */
export function spawnSubagentsTool(opts, ctx = DEFAULT_TOOL_CONTEXT) {
    const maxDepth = opts.maxDepth ?? DEFAULT_MAX_SUBAGENT_DEPTH;
    const parentDepth = opts.currentDepth ?? 0;
    const maxParallel = opts.maxParallel ?? DEFAULT_MAX_PARALLEL_SUBAGENTS;
    return tool({
        name: 'spawn_subagents',
        description: 'Delegate MULTIPLE focused subtasks to child agents in parallel. Each spec runs as its own subagent (same per-element schema as `spawn_subagent`). Results are returned in submission order regardless of completion order. Per-child failures isolate (the others keep running). Aggregate cost and token totals sum across successful children only. Use when several subtasks are independent and can run concurrently — for sequential delegation, prefer `spawn_subagent`. Returns `{ results, aggregatedUsage, durationMs }`.',
        inputSchema: z.object({
            subagents: z
                .array(SPAWN_SUBAGENT_INPUT_SCHEMA)
                .min(1)
                .max(MAX_PARALLEL_BATCH_SIZE)
                .describe(`Array of subagent specs (1 to ${MAX_PARALLEL_BATCH_SIZE}). Each element accepts the same fields as the singular spawn_subagent tool — required \`description\` plus optional per-spec overrides (\`tools\` / \`instructions\` / \`max_turns\` / \`max_budget_usd\` / \`model\` / \`permission_mode\` / \`allowed_tools\` / \`disallowed_tools\` / \`effort\`). Concurrency is capped at the factory's \`maxParallel\` (default ${DEFAULT_MAX_PARALLEL_SUBAGENTS}); larger arrays queue.`),
        }),
        execute: async ({ subagents }, execCtx) => {
            ...
            for (const r of results) {
                if (r.status !== 'success')
                    continue;
                if (r.output.costUsd !== undefined)
                    usd += r.output.costUsd;
